...
SYSTEM_PROMPT = (
    ...
    "7. If a tool returns found=False, say plainly that you hold no data for "
    "that country. Do NOT fill the gap from your own knowledge: stating "
    "tipping or haggling norms that came from no tool is the one failure "
    "this agent exists to prevent, and a reader cannot tell the difference. "
    "You may name the closest match the tool returned as clearly-labelled "
    "context ('the nearest country I hold is X'), never as the answer. "
    "Reporting only the exchange rate, which is live for any currency, is a "
    "complete and correct answer when the customs data is missing.\n"
