...
def _build_agent(model_string: str):
    """Construct the agent on a named model.

    Prompt, tools and routing logic are exactly as before -- only the model
    string is a parameter now. Split out so the model can be re-read at call
    time: DESTINATION_AGENT_MODEL was read once, at import, which made the slot
    configurable at startup but left the orchestrator's provider fallback unable
    to reach it. That fallback repoints the variable and re-imports this module,
    and a re-import is a no-op once Python has cached it -- so a run on 27 Aug
    logged 'provider unavailable; retrying' and then failed again on the same
    dead provider. Agreed with the owner, 27 Aug 2026.
    """
...
_model_string = os.environ.get("DESTINATION_AGENT_MODEL", _DEFAULT_MODEL)
agent = _build_agent(_model_string)
...
def run_destination_agent(user_query: str) -> str:
    ...
    # Re-read the model here, not only at import. The orchestrator's provider
    # fallback repoints DESTINATION_AGENT_MODEL and rebuilds the client; without
    # this the rebuilt client was the same cached module object on the same dead
    # provider. Costs one dict lookup per call and rebuilds only when the value
    # actually changed, so a normal run is unaffected.
    global agent, _model_string
    _wanted = os.environ.get("DESTINATION_AGENT_MODEL", _DEFAULT_MODEL)
    if _wanted != _model_string:
        agent, _model_string = _build_agent(_wanted), _wanted
